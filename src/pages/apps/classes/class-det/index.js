...
import { deleteClass } from 'src/store/apps/class'
...
const Class_Det = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`http://localhost:1337/api/classes/${router.query.id}`)
  ...
  const handleDeleteClass = async () => {
    ...
    router.push('/apps/classes') // Redirect to the student list page after deletion
  }
  ...
  return (
    <Box sx={{ flexGrow: 1, backgroundColor: '#EEEFF7', p: 4 }}>
      <Card fullwidth>
        <CardHeader
          avatar={<Box sx={{ width: 8, backgroundColor: '#FFA000', borderRadius: 3, height: 45 }}></Box>}
          action={
        ...
        <CardContent>
          ...
          <Grid container sx={{ pl: 4, pt: 2 }}>
            ...
            <Grid xs={2}>
              <Typography>{data.roomNo}</Typography>
            </Grid>
          </Grid>
          <Grid container sx={{ pl: 4, pt: 2 }}>
            <Grid xs={2}>
            ...
            <Grid xs={2}>
              <Typography>{data.numberOfStudent}</Typography>
            </Grid>
          </Grid>
          <Grid container sx={{ pl: 4, pt: 2 }}>
            <Grid xs={2}>
        ...
        <CardContent>
          <Accordion sx={{ border: 'none' }} variant='text' elevation={0}>
            <AccordionSummary expandIcon={<ExpandMoreIcon />} aria-controls='students_summary' id='students_header'>
              <Stack direction='row'>
        ...
        <CardContent>
          <Stack direction='row' sx={{ pl: 5 }}>
            ...
            <Button sx={{ flex: 'end' }} aria-describedby={id} onClick={handleClick}>
              <MoreVert></MoreVert>
            </Button>
          </Stack>
          <Divider />
      ...
      >
        <Stack sx={{ p: 2 }}>
          <Button varient='text' sx={{ textAlign: 'left' }}>
            Add Subject
          </Button>
          <Button varient='text' sx={{ textAlign: 'left' }}>
            Manage Subjects
      ...
      >
        <Stack sx={{ p: 2 }}>
          ...
          <Button
            varient='text'
            sx={{ color: '#FF0000', borderColor: '#FF0000', border: 1, textAlign: 'left' }}
